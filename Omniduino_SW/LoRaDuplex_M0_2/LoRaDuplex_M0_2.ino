...
byte localAddress = 0x55; //**Device's Address, different for every device on the network or "This message is not for me" will be shown
byte destination = 0xAA;  //**Address for destination device
...
void loop() {
  if (millis() - lastSendTime > interval) {
    String message = "HeLoRa from node_2!";   // send a message
    sendMessage(message);
